  ...
  return (
    <div className="beading-sheet-backdrop" role="presentation" onClick={onClose}>
      <section className="beading-sheet inventory-check-sheet" role="dialog" aria-modal="true" aria-label="库存检测" onClick={(event) => event.stopPropagation()}>
        ...
        {onWarehouseChange ? <label className="beading-warehouse-select">选择仓库<select value={warehouseId} onChange={(event) => onWarehouseChange(event.target.value)}><option value="">不使用仓库</option>{(warehouseOptions || []).map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.name}</option>)}</select></label> : null}
